...
function initTeXView() {
    ...
    if (!isWeb) {
        ...
        httpRequest.onreadystatechange = function () {
            if (httpRequest.readyState === 4 && httpRequest.status === 200) teXView.appendChild(createView(JSON.parse(httpRequest.responseText)));
...
function initWebTeXView(id, rawTeXHTML) {
    ...
    document.querySelectorAll("flt-platform-view").forEach(function (platformView) {
            ...
            if (view.id === 'tex_view_' + id) {
                ...
                webTeXView.appendChild(createView(rawTeXHTML))
