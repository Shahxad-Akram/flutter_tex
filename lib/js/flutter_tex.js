...
function initWebTeXView(viewId, rawData, engine) {
    ...
    if (iFrameWeb != null) {
        ...
        if (iFrame != null) {
            ...
            texViewMap[viewId] = teXView;
            if (teXView != null) {
                teXView.innerHTML = "";
                initiated = true;
                var jsonData = JSON.parse(rawData);
                teXView.appendChild(createTeXView(jsonData));
                iFrame.renderTeXView(() => {
                    renderCompleted(viewId);
                });
            }
        }
    }
...
function renderCompleted(viewId = null) {
    var currentView = isWeb ? texViewMap[viewId] : teXView;
    var height = getTeXViewHeight(currentView);

    if (renderingEngine = "katex") height += 5;

    if (isWeb) {
        if (viewId != null) {
            var callback = window['TeXViewRenderedCallback_' + viewId];
            if (callback) {
                callback(height);
            }
        } else {
